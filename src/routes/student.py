...
def get_stats():
    if not students_db:
        return {"totalStudents": 0, "averageGrade": 0, "studentsByField": {}, "bestStudent": None}
...
def search_students(q: str = Query(default=None)):
    ...
    if not q or not q.strip():
        raise HTTPException(status_code=400, detail="Le paramètre 'q' est obligatoire et ne peut pas être vide.")
...
@router.post("/", response_model=Student, status_code=status.HTTP_201_CREATED, include_in_schema=False)
